...
class TypeChoosePageViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func playButtonTapped(_ sender: UIButton) {
        ...
        ref.child("questionBanks").child("mandarin").child("genreCod1").child("question1").queryOrderedByKey().queryLimited(toFirst: 5).observeSingleEvent(of: .value, with: { (snapshot) in
